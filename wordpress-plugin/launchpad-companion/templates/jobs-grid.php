<?php
/**
 * The Job Capture grid — rendered by the [pig_jobs] shortcode and the launchpad/pig-jobs block. Runs in
 * {@see \Launchpad\Companion\Render\JobRender::render()}'s scope, where $query is the WP_Query of published
 * pig_job posts. Theme-agnostic markup + scoped inline styles (printed once).
 *
...
if (! $query->have_posts()) {
    echo '<p class="pig-jobs-empty">No recent jobs to show yet.</p>';
    ...
    while ($query->have_posts()) :
        ...
        <article class="pig-job-card">
            ...
            <div class="pig-job-body">
                ...
                <?php $cities = get_the_term_list(get_the_ID(), \Launchpad\Companion\Content\JobCpt::TAX_CITY, '', ', '); ?>
