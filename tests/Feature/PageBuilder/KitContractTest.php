...
test('the seeder persists the library kits idempotently (keyed by name, not page_type)', function () {
    ...
    // 2 locked page kits + 7 standard-page composer kits + 1 hub kit; re-seed updates rather than duplicates.
    expect(WireframeKit::whereNotNull('page_type')->count())->toBe(10)
        ->and(WireframeKit::where('page_type', 'utility')->count())->toBe(6)  // about / why-choose-us / areas-we-serve / faq / privacy / terms share page_type
