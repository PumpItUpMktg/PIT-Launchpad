...
function absentPrompt(Site $site, Service $svc, Market $market, array $competitors = ['Rival']): GeoPrompt
{
    $p = GeoPrompt::create([
        'site_id' => $site->id, 'service_id' => $svc->id, 'market_id' => $market->id,
...
function tuSite(): array
{
    ...
    return [$site, Service::factory()->create(['site_id' => $site->id, 'name' => 'Repair']), Market::factory()->priority()->create(['site_id' => $site->id, 'name' => 'Union'])];
}
...
it('generates assisted variants for an absent gap, tagged with the parent dimensions', function () {
    [$site, $svc, $market] = tuSite();
    absentPrompt($site, $svc, $market);
    ...
    expect($variants)->toHaveCount(2)
        ...
        ->and($variants->first()->market_id)->toBe($market->id)
