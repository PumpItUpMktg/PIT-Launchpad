<?php
/**
 * Minify the theme's readable source scripts → their .min builds. String- and comment-aware in one ordered
 * pass: `/* … *\/` and `// …` comments are dropped, but string literals ("…", '…', `…`) are protected FIRST
 * so a `//` inside a URL (e.g. "https://{s}.basemaps…") is never mistaken for a comment. Only whitespace
 * runs are collapsed (comments removed) — operators and statement boundaries are left intact, so the result
 * stays ASI-safe. The theme lazy-/footer-loads the .min builds; the readable sources stay the truth.
 *
