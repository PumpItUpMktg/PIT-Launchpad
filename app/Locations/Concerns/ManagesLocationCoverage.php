...
namespace App\Locations\Concerns;
...
/**
 * The locations + coverage workspace, extracted so two Livewire surfaces share it verbatim:
 * the guided WhereYouWork step (setup) and the Settings Locations page (the post-setup editor).
 * A list of base locations (each: where it is + which counties it serves + located status),
 * a single "Add location" flow, county-based coverage (tiered towns, page_selected), and the
 * shared coverage map. Owner framing throughout — "where you are" / "counties you serve",
 * never "radius / lat/lng".
 *
 * The host page declares `public ?string $siteId` (GuidedPage already carries it) and decides
 * how the site is chosen; everything else lives here.
 *
...
trait ManagesLocationCoverage
{
    ...
    protected function buildCoverage(bool $notify): void
    {
        $site = $this->coverageSite();
        if ($site === null) {
            return;
    ...
    protected function autoGeocodePending(): void
    {
    ...
    private function coverageLocation(string $locationId): ?Location
    {
    ...
    private function coverageSite(): ?Site
    {
