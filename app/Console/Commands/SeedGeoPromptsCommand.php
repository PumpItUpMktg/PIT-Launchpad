...
class SeedGeoPromptsCommand extends Command
{
    ...
    protected $signature = 'sandhog:seed-geo-prompts {site : Site id, brand name, or domain (partial ok)}
        ...
        {--refresh : Re-render existing auto-seeded prompts from the current town names (fixes stale text after cleaning names)}';
    ...
    public function handle(GeoPromptSeeder $seeder): int
    {
        ...
        if ($this->option('refresh')) {
            ...
            $this->line(sprintf('<info>%s</info> — %d prompt(s) refreshed of %d checked.', $site->brand_name, $r['updated'], $r['checked']));
